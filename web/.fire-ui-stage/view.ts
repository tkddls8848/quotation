...
export function mountFire(root: HTMLElement): void {
  ...
  const tabList = create('div', 'fire__input-tabs');
  tabList.setAttribute('role', 'tablist');
  tabList.setAttribute('aria-label', '계산 조건');
  const groups = ['기본 정보', '지출과 가족', '연금과 가정'].map((title, index) => {
    const button = create('button', 'fire__input-tab', title);
    button.type = 'button';
    button.id = `fire-input-tab-${index}`;
    button.setAttribute('role', 'tab');
    button.setAttribute('aria-controls', `fire-input-panel-${index}`);
    const panel = create('div', 'fire__input-panel');
    panel.id = `fire-input-panel-${index}`;
  ...
  const navigation = create('div', 'fire__navigation');
  const previous = create('button', 'fire__secondary', '이전');
  const next = create('button', 'fire__next', '다음');
  previous.type = next.type = 'button';
  let activeGroup = 0;
  const selectGroup = (index: number, focus = false): void => {
    activeGroup = index;
    groups.forEach(({ button, panel }, position) => {
    ...
    next.textContent = index < groups.length - 1 ? `다음: ${groups[index + 1]!.button.textContent}` : '마지막 항목';
    if (focus) groups[index]!.button.focus();
  };
  groups.forEach(({ button }, index) => {
    button.addEventListener('click', () => selectGroup(index));
    button.addEventListener('keydown', (event) => {
      let target: number;
      if (event.key === 'ArrowRight') target = (index + 1) % groups.length;
      else if (event.key === 'ArrowLeft') target = (index + groups.length - 1) % groups.length;
  ...
  previous.addEventListener('click', () => selectGroup(activeGroup - 1, true));
  next.addEventListener('click', () => selectGroup(activeGroup + 1, true));
  navigation.append(previous, next);
  form.append(tabList, create('p', 'fire__auto-note', '입력 즉시 결과에 반영됩니다. 다른 항목으로 이동해도 입력값은 유지됩니다.'), ...groups.map(({ panel }) => panel), navigation);
  selectGroup(0);
  ...
  const assetMore = create('details', 'more');
  assetMore.append(create('summary', 'more__summary', '배당주 성장률 조정'));
  assetMore.append(
    fieldRow(
  ...
  for (const [index, section] of SECTIONS.entries()) {
    ...
    const group = index < 2 ? 0 : index < 4 ? 1 : 2;
    groups[group]!.panel.append(card);
  }
  groups[0]!.panel.append(assetCard);
...
function drawResult(root: HTMLElement, input: FireInput, result: FireResult): void {
  ...
  const expanded = previousDetails?.open ?? window.matchMedia('(min-width: 62rem)').matches;
  const tableExpanded = root.querySelector<HTMLDetailsElement>('.chart details')?.open ?? false;
  ...
  const parts: HTMLElement[] = [stats, chart(input, result)];
  ...
  const details = create('details', 'fire__result-details');
  details.open = expanded;
  details.append(create('summary', 'fire__disclosure', '상세 결과 · 자산 추이 보기'));
  const content = create('div', 'fire__result-content');
  content.append(...parts);
  details.append(content);
  const chartTable = content.querySelector<HTMLDetailsElement>('.chart details');
  if (chartTable) chartTable.open = tableExpanded;
  root.replaceChildren(box, details);
}

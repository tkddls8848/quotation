...
def test_default_template_ships_with_the_core_package():
    for mode in resources.TEMPLATE_NAMES:
        assert resources.default_template_path(mode).is_file()
        assert resources.default_template_bytes(mode)[:2] == b"PK", \
            "xlsx(zip) 여야 한다"


def test_there_are_exactly_the_two_supported_templates():
    """양식은 IBM 용·레노버 x86 용 둘뿐이다. 데스크톱과 웹이 같은 파일을 쓴다.
    ...
    assert books == sorted(resources.TEMPLATE_NAMES.values()), \
        f"지원하는 양식과 다르다: {books}"

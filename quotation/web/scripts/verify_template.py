...
def template_original(mode: str) -> Path:
    return FEATURE / "python" / "quotation" / "resources" / TEMPLATE_NAMES[mode]
...
def adopt(source: Path, mode: str) -> int:
    ...
    for script in ("build_browser_engine.py",):
        path = FEATURE / "web" / "scripts" / script
        if not path.is_file():
            continue
        print(f"\n=== {script}")

...
만드는 것 — 전부 `web/public/engine/` 아래, 저장소에서 추적하지 않는다.

    quotation_wasm.js           wasm-bindgen 글루 (ES 모듈)
    quotation_wasm_bg.wasm      변환 엔진
    engine.json                 판본·해시 (화면과 테스트가 읽는다)

    python quotation/quotation/web/scripts/build_browser_engine.py
...
#: quotation/web — 견적기의 웹 부분
WEB = Path(__file__).resolve().parents[1]
#: quotation — 견적기 한 덩어리 (코어·rust·데스크톱·웹이 다 여기 있다)
FEATURE = WEB.parent
#: 저장소 뿌리. Cargo 작업공간과 배포 셸(web/)이 여기 있다.
ROOT = FEATURE.parent
CRATE = FEATURE / "rust" / "wasm"
PKG = CRATE / "pkg"
#: 셸이 정적 자산으로 내보내는 자리.
...
def template_facts() -> dict[str, dict]:
    ...
    sys.path.insert(0, str(FEATURE / "python"))
    from quotation.core.resources import TEMPLATE_NAMES

    facts = {}
    for mode, name in TEMPLATE_NAMES.items():
        raw = (FEATURE / "python" / "quotation" / "resources" / name).read_bytes()
        digest = _sha256(raw)
        facts[mode] = {
            "name": name,
            "sha256": digest,
...
def stale() -> list[str]:
    ...
    if not (PKG / WASM).is_file():
        return ["quotation/rust/wasm/pkg (wasm-pack 산출물이 없습니다)"]
...
def main() -> int:
    ...
    if args.check:
        ...
        print("엔진 자산이 최신입니다." if not problems else
              "python quotation/quotation/web/scripts/build_browser_engine.py 를 다시 실행하십시오.")

 ...
 * 견적서 변환 화면 (계획서 §7).
 *
 * 화면 뼈대(`panel.html`)와 스타일(`quotation.css`)까지 이 폴더가 갖는다. 셸은
 * 빈 칸 하나만 내주고, 이 기능이 그 안을 채운다 — 견적서 화면을 고칠 때 다른
 * 기능의 파일을 건드릴 일이 없게 하기 위함이다.
 *
 * 규칙 몇 가지를 코드로 못박아 둔다.
 *   - 브라우저 1차 검사는 편의일 뿐이고 판단은 변환 코어가 다시 한다
 ...
 * 화면을 세우고 배선한다.
 *
 * @param root 셸이 내준 빈 칸
 * @param deploymentVersion 꼬리말에 찍을 배포 판본 (셸이 빌드 시점에 받는다)
 */
export function mountQuotation(root: HTMLElement, deploymentVersion: string): void {
  ...
  // --- 처음 그리기 -----------------------------------------------------------
  //
  // 템플릿 판본은 여기서 미리 보여 주지 않는다 — IBM 문서와 Lenovo 문서가
  // 템플릿이 서로 달라서, 무엇을 변환할지 모르는 시점에는 "그" 판본이라는 게
  // 없다. 실제로 쓴 판본은 변환이 끝난 뒤에 알 수 있다.

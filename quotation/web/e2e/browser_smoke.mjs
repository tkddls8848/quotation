 ...
 * 변환 버튼을 눌러 실제로 내려받는다.
 *
 *   node quotation/web/e2e/browser_smoke.mjs <dist> <출력폴더> <xml…>
 *
...
/** 운영에서 붙는 헤더 (web/public/_headers 와 같은 값). */
const CSP =
...
if (!existsSync(join(dist, 'engine', 'engine.json'))) {
  console.error(`${dist}/engine 에 변환 엔진이 없습니다. `
    + 'python quotation/web/scripts/build_browser_engine.py 뒤에 화면을 빌드하십시오.');
  process.exit(2);
}

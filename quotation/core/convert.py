...
from .pricing import parse_discount
...
def output_path_for(xml_path: Path, out_dir: Path | None = None) -> Path:
    """출력 파일 경로. 원본과 같은 이름에 확장자만 .xlsx."""
    folder = Path(out_dir) if out_dir else xml_path.parent
    return folder / f"{xml_path.stem}.xlsx"
...
def convert(xml_path: str | Path, *, out_dir: str | Path | None = None,
    ...
    Raises:
        ...
        pricing.DiscountError: 할인율 입력 문제

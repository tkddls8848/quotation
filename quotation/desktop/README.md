...
견적기의 공용 코어 `quotation/python/quotation/core` 에 있습니다.
...
.\.venv\Scripts\python.exe quotation\desktop\launcher.py            # 개발 실행
.\.venv\Scripts\python.exe -m pytest quotation\desktop\tests -q     # 데스크톱 테스트
```

`launcher.py` 는 견적기의 파이썬 뿌리와 `quotation/desktop/` 을 `sys.path` 에 얹으므로 별도 설치
없이 그대로 실행됩니다.
...
산출물은 `quotation\desktop\dist`, PyInstaller 중간물은 `quotation\desktop\build` 에 냅니다. 둘 다
데스크톱 전용이라 저장소 루트를 어지럽히지 않게 여기로 모았습니다. `dist` 폴더
자체는 `.gitkeep` 으로 남기고 내용물은 추적하지 않습니다.

`quotation\desktop\dist\QuotationTool.exe` 하나만 배포하면 됩니다. 처음 실행할 때 EXE 옆에
...
파일이 자동으로 골라집니다. 원본은
`quotation/python/quotation/resources/견적서_template_{IBM,Lenovo}.xlsx` 이며 spec 이 번들에
넣습니다.
...
개발 실행에서는 EXE 옆이 `quotation/desktop/` 폴더입니다. 이 사본은 추적하지 않습니다.

## 웹 앱과의 관계

계획서 §11 Phase 6 에 따라 웹 앱 전환 뒤에도 데스크톱 앱을 일정 기간 복구
수단으로 유지합니다. 두 경로가 같은 결과를 내는지는
`quotation/tests/test_bytes_api.py` 가 셀 단위로 대조합니다.

...
### Phase 3 — 출력 구현 ✅ **완료 (2026-07-23)**
- [x] 템플릿 `.xls` → `.xlsx` 변환 및 서식 보존 검증 (`tools/xls2xlsx.ps1`)
- [x] `ibm_writer` — TOTAL + 장비군 시트 + 서식/글꼴/병합/인쇄영역 + H열(MA) + I열(MA 기간)
- [x] 골든 대비 셀별 diff 테스트 — **두 샘플 모두 차이 0건**
- [x] Excel 재계산 검증 (`tools/verify_in_excel.ps1`) — 총합계·MA 계산값 전부 일치
- [x] 성능 측정 (`tools/benchmark.py`)

| 항목 | 결과 |
|---|---|
| 변환 시간 | **0.15~0.16초** (목표 1~3초 대비 초과 달성) |
| Excel 프로세스 | **사용 안 함** — 테스트 전체가 Excel 없이 통과 |
| 골든 일치 | 차이 0건 / 예외 1건(`SERVER 1!D20`, 골든 수기 편집 추정) |
| 테스트 | 48건 통과 |

**Phase 3 에서 확정된 사양** (`SPEC_CELLMAP.md` §4.4~§4.7)
- H/W 구간은 블록별 합계행을 갖고 열거형 SUM, S/W 구간은 블록 합계 없이 범위형 SUM
- 서브라인 수량 E열이 3형태(`=8*E8` / `=1` / `1`)로 갈리는 규칙
- 서브라인 0건 블록의 스페이서 행
- 글꼴 규칙 (데이터 Tahoma 9 / 한글 합계 라벨 돋움 9 볼드 / C1 Tahoma 18 / C3 HY헤드라인M 9)
- `Services` 는 S/W 구간

### Phase 4 — UI 및 부가 기능 ✅ **완료 (2026-07-23)**
- [x] 메인 화면 재구성 (`ui/main_window.py`, tkinter/ttk)
- [x] 진행률 표시 — 원본 ProgressBar 대응. **변환을 작업 스레드로 돌려 UI 가 멈추지 않는다**
- [x] 설정 마이그레이션 (`견적서생성기.ini` cp949 → `%LOCALAPPDATA%\QuotationTool\config.json`)
- [x] 로그 (`%LOCALAPPDATA%\QuotationTool\logs\`, 일자별 30일 보관)
- [x] 예외 처리 — 파일 잠김/XML 오류/할인율 오류를 원본 문구로 안내
- [x] 결과 폴더 자동 열기, 최근 경로·할인율·옵션 기억
- [x] **CLI 일괄 변환** (원본에 없던 기능): `QuotationTool.exe a.xml b.xml -o out\`
- [x] 템플릿을 패키지 리소스로 동봉 (`quotation/resources/`)
- [x] 테스트 66건 통과 (코어 46 + writer 2 + 앱 15 + UI 스모크 3)

**GUI 선택: tkinter/ttk** — 표준 라이브러리라 추가 의존성이 없고 PyInstaller 번들이
작다(PySide6 대비 약 1/4). 원본 Form1 이 단순한 폼이라 기능상 부족함이 없다.

**원본 대비 개선**
| 항목 | 원본 | 현재 |
|---|---|---|
| 변환 중 UI | 멈춤 | 작업 스레드, 진행률 표시 |
| 파일 잠김 | "Excel을 모두 종료" | 해당 견적서만 닫으면 됨 |
| 설정 위치 | `Program Files` (쓰기 차단→유실) | `%LOCALAPPDATA%` |
| 인코딩 | ANSI INI | UTF-8 JSON |
| 장애 추적 | 불가 | 로그 파일 |
| 일괄 처리 | 불가 | CLI 지원 |

**정의만 하고 골든으로 검증하지 못한 동작** (샘플 부재)
- `유지정비료 포함` 해제 시 H·I열을 비운다 (기본값 켜짐 = 골든과 일치)
- 할인율 입력 시 `공급가` 행에 `=G{총합계행}*{1-할인율}` 기록. 0이면 공란(골든과 일치)

### Phase 5 — 검증 및 배포 ✅ **완료 (2026-07-23)**
- [x] 회귀 테스트: 골든 2건 대비 차이 0건 (예외 1건은 근거와 함께 기록)
- [x] 성능 측정: **0.2초**
- [x] Excel 을 띄운 상태에서 변환 → 충돌 없음 확인
- [x] PyInstaller 단일 EXE 빌드 (64bit, 16MB, 무설치)
- [x] 한글·공백·특수문자 경로 테스트
- [x] 인수 시험 자동화 (`tools/acceptance.ps1`) — **14항목 전부 통과**
- [x] `README.md`, `MIGRATION.md` 작성

**산출물**
| 파일 | 용도 |
|---|---|
| `dist/QuotationTool.exe` | GUI. XML 을 끌어다 놓으면 채워진 채로 열린다 |
| `dist/QuotationTool-cli.exe` | 일괄 변환. 종료 코드를 돌려준다 |

> EXE 를 둘로 나눈 이유: PyInstaller 의 windowed 빌드는 콘솔이 없어 PowerShell/cmd 가
> 종료를 기다리지 않고 파이프에 물리면 호출 측이 멈춘다. 자동화에는 콘솔 빌드가 필요하다.

**인수 시험 항목** (`tools/acceptance.ps1`)
1. EXE 2종 존재 · 2. 한글 경로 일괄 변환 · 3. **EXE 산출물의 골든 대조**
4. 오류 처리(잘못된 XML/할인율, 종료 코드, 원본 문구 유지)
5. **Excel 실행 중 변환** 및 잠긴 대상의 명확한 실패 · 6. GUI 기동과 잔류 프로세스 없음

**빌드 중 해결한 문제**
- `quotation/__main__.py` 를 진입점으로 주면 패키지 컨텍스트가 없어 상대 임포트가 깨진다 → `launcher.py` 추가
- 콘솔 코드페이지(cp949) 와 Python UTF-8 출력 불일치로 한글이 깨짐 → 콘솔 CP 에 맞춰 재설정
- onefile EXE 는 부트로더가 자식 프로세스로 앱을 띄운다. 창을 가진 것은 자식이다

...
.\.venv\Scripts\python.exe desktop\launcher.py            # 개발 실행
.\.venv\Scripts\python.exe -m pytest desktop\tests -q     # 데스크톱 테스트
```

`launcher.py` 는 저장소 루트와 `desktop/` 을 `sys.path` 에 얹으므로 별도 설치
없이 그대로 실행됩니다.
...
개발 실행에서는 EXE 옆이 `desktop/` 폴더입니다. 이 사본은 추적하지 않습니다.

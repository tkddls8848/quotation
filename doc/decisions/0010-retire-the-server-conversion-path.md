...
화면 쪽 호출부(`quotation/web/src/api.ts` — 지금은 `contract.ts` — 의 `convertOnServer` 와 그리로 넘어가던
분기)는 이때 함께 지우지 않았다. 받아 줄 서버가 없으니 엔진을 못 띄운 브라우저는
XML 을 올리고 정적 자산 처리기의 `index.html` 을 받는 것으로 끝났다 — 변환은
되지 않는데 파일만 나가는 셈이었다. **2026-09-15 그 호출부까지 지웠다.** 이제
엔진을 못 띄우면 그 사실을 알리고, 파일은 어디로도 보내지 않는다.

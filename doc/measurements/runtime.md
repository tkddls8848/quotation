...
| 항목 | Pyodide (지금) | Rust→WASM (관문 측정용) | Rust→WASM (완성된 코어) | 판정 기준 |
|---|---:|---:|---:|---|
| 전송 자산 (gzip) | 14.4 MiB | 0.799 MiB | **1.012 MiB** | 1.1 MiB 이하 — 통과 ([결정 0008](../decisions/0008-wasm-size-after-the-writer.md)) |
| 기동 (compile + instantiate) | 약 2,500 ms | 4.2 ms | **5.1 ms** | 200 ms 이하 — 통과 |
| 변환 1건 | 약 300 ms | 78 ms (부분 작업) | **188 ms** (실제 견적서) | 지금보다 느려지면 중단 |

가운데 열은 견적서를 만드는 코드가 들어가기 전의 값이다. 오른쪽 열이 지금의
코어다 — XML 을 읽어 견적서 파일을 통째로 만든다.
...
`3` 은 크기 기준을 넘긴다. 그래서 `"z"` 를 쓴다. **계획 §1 이 추정한 변환

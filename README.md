...
| 유지정비료 포함 | 해제하면 H·I열(제안가/유지정비료)을 비운다 |
| 완료 후 폴더 열기 | 탐색기에서 결과 화일을 선택된 상태로 연다 |

### CLI (일괄 변환)
```
QuotationTool-cli.exe a.xml b.xml -o 출력폴더
QuotationTool-cli.exe *.xml -d 15.5 --no-maintenance
```
| 옵션 | 설명 |
|---|---|
| `-o, --out-dir` | 저장 폴더 (기본: XML 과 같은 폴더) |
| `-d, --discount` | 할인율 % |
| `--no-maintenance` | 유지정비료를 채우지 않음 |
...
무설치. EXE 하나만 있으면 된다. **Excel 이 설치되어 있지 않아도 동작한다.**
